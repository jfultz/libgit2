...
int openssl_connect(git_io_stream *stream)
{
	int ret;
	openssl_stream *st = (openssl_stream *) stream;

	if ((ret = git_io_stream_connect((git_io_stream *)st->socket)) < 0)
		return ret;
...
ssize_t openssl_write(git_io_stream *stream, void *data, size_t len, int flags)
{
	openssl_stream *st = (openssl_stream *) stream;
	int ret;
	size_t off = 0;
...
ssize_t openssl_read(git_io_stream *stream, void *data, size_t len)
{
	openssl_stream *st = (openssl_stream *) stream;
	int ret;
...
int openssl_close(git_io_stream *stream)
{
	openssl_stream *st = (openssl_stream *) stream;
	int ret;
	...
	return git_io_stream_close((git_io_stream *)st->socket);
}

void openssl_free(git_io_stream *stream)
{
	openssl_stream *st = (openssl_stream *) stream;

	git_io_stream_free((git_io_stream *) st->socket);
	git__free(st);
}

int git_openssl_stream_new(git_io_stream **out, const char *host, const char *port)
{
	int ret;
	openssl_stream *st;
	...
	if (git_socket_stream_new((git_io_stream **) &st->socket, host, port))
		return -1;
	...
	if ((ret = SSL_set_fd(st->ssl, st->socket->s)) <= 0) {
		openssl_close((git_io_stream *) st);
		return ssl_set_error(st->ssl, ret);
	}
